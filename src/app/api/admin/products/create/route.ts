...
const productSchema = z.object({
  ...
  sku: z.string().optional(), // SKU é opcional
...
export async function POST(request: Request) {
  try {
    ...
    // 3. Verificar duplicidade (Nome Único)
    const existing = await prisma.productCatalog.findFirst({
      where: { name: parsedData.name }
    });
    if (existing) {
      return NextResponse.json({ message: 'Um produto com este Nome já existe.' }, { status: 409 });
    }
